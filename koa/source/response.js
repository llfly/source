...
module.exports = {
  ...
  set status(code) {                  //设置状态
    ...
    if (this.body && statuses.empty[code]) this.body = null;    //204，205，304强改返回体为null
  },
  ...
  set body(val) {
    ...
    const setType = !this.header['content-type'];        //如果没有设置content-type
    ...
    // json
    this.remove('Content-Length');                        //json
    this.type = 'json';
  },
  ...
  vary(field) {                                       //相当于执行res.append('Vary', field)。
    vary(this.res, field);
  },
  ...
  append(field, val) {                  //调用设置方法,追加设置头部
